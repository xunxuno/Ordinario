...
router.post('/gestion', async (req, res) => {
    try {
        ...
        // Renderizar la vista con los datos del vuelo
...
router.post('/gestion/:id', async (req, res) => {
    try {
        ...
        // Aquí puedes manejar la lógica para gestionar el vuelo con el ID proporcionado
        //res.send(`Gestionando vuelo con ID: ${vueloId}`);
    } catch (error) {
        console.error('Error al gestionar el vuelo:', error);
        res.status(500).send('Error interno del servidor');
    }
    try {
        // Extraer los datos del formulario
